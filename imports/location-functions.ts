...
export const locationFunctions = {
	...
	coordsToWP: ([x, y, z]: [
		...
		new net.runelite.api.coords.WorldPoint(x, y, z), // Convert coordinates to WorldPoint

	// Get distance from local player to WorldPoint
	localPlayerDistFromWP: (
		worldPoint: net.runelite.api.coords.WorldPoint,
	): number =>
		client.getLocalPlayer().getWorldLocation().distanceTo(worldPoint), // Get distance from local player to WorldPoint
	...
	isPlayerNearWP: (
		...
		locationFunctions.localPlayerDistFromWP(worldPoint) <= tileThreshold, // Check if player is near WorldPoint within tile threshold, 5 tiles by default
	...
	wWalkTimeout: (
		...
		// Web walk to WorldPoint with timeout
		const isPlayerAtLocation = () =>
			locationFunctions.isPlayerNearWP(worldPoint, targetDistance);
